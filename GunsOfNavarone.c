...
// A square also requires a flag to indicate use in the current word being
// formed, the number of valid neighbours, and the index of the letter showing
// on its face.
struct square {
  // The Used flag will indicate if a square is being used in constructing the
  // current word, and hence to remove the used square from further inclusion in
  // the same word.
  Bool Used;
...
// This initialization function sets the neighbour array for all of the Squares
// in ThisBoard's Block (this only needs to be done once). The Letter index for
// each square will be a blank space ' ', and they will all be not Used.
...
// This function simply transcribes the BoardString data into ThisBoard using
// the correct format. A major optimization has taken place at this level
// because the ADTDAWG's direct property enforces the "Order Does Not Matter,"
// paradigm, and thus, no sorting is required.
...
// This function will be used to debug the program.  It will not be used when
// the analysis of a large number of boards is required.
...
// Each worker thread will have it's own time stamping unsigned int array for
// all of the words in the lexicon.
...
// An explicit stack is used to traverse the neighbours of "BeginSquare,"
// regardless of alphabetical order.  It updates the global "LexiconTimeStamps"
// to eliminate the counting of identical words. The algorithm is inherently
...
int SquareWordDiscoverStack(
  ...
  while (DISCOVERY_STACK_NOT_EMPTY) {
    ...
    if (FirstTime == TRUE) {
      ...
      // Check to see if we have arrived at a new word, and if so, add the
    ...
    // ("NumberOfLivingNeighbours" - 1) to zero.  There will be no scrolling
    // through a list of children in the lexicon data structure when using the
    // ADTDAWG.
    DoWeNeedToPop = TRUE;
    ...
    if (DoWeNeedToPop) {
      // We have now finished using "WorkingSquare", so set its "Used" element
      // to FALSE.
...
// Converts the two digit integer X into a string that tacks onto board strings
// to indicate the last square to be altered.
