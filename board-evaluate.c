...
// paradigm, and thus, no sorting of neighbours is required.
...
// The ADTDAWG board analysis scheme is extremely powerful when applied to
...
// "BeginSquare," regardless of alphabetical order.  It updates the global
// "LexiconTimeStamps" to eliminate the duplicate word problem. The algorithm is
// inherently recursive, and this drawback has been excised. Every letter on the
...
int SquareWordDiscoverStack(SquarePtr BeginSquare, unsigned int BeginIndex,
  ...
  while (DISCOVERY_STACK_NOT_EMPTY(TheTop)) {
    ...
    // printf("2\n");
    //  If "WorkingSquare" has children, visit the next one up to
    //  "NumberOfLivingNeighbours" - 1.  There will be no scrolling through a
    ...
    if (DoWeNeedToPop) {
      // We have now finished using "WorkingSquare", so set its Used element to
      // FALSE.
